...
| UI/UX | mobile実機、キーボード操作、読み上げ、色コントラストの確認 | 画面上は良くなった。誰が使っても困りにくいかは、もう少し別の確認が必要 |
| ハーネス実験素材 | 失敗ケース、再発検知、同じタスクの再試行データ | チェックはかなりある。5 / 5 にするには、失敗も拾えるか、次回も同じ品質になるかを見る必要がある |
