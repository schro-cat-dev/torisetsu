...
- ただし、今回の品質評価はローカル実験用TODOアプリとしての評価です。外部ユーザーへ公開して運用する品質の評価はしていない。認証、DB、権限、複数ブラウザ、実運用監査は未対応。
...
| 外部公開・本番運用 | 対象外 | 認証、DB、権限、監査、複数環境確認は今回見ていない |
| 再現性 | 4 / 5 | docs、skill、chat log、harness_runsに残っている。総作業時間だけ未計測が残る |

### 1.1 5 / 5 に足りないもの

今の目的にはかなり届いているが、まだ見ていない範囲がある、という意味です。

| 視点 | 5 / 5 に足りないもの | かんたんな説明 |
|---|---|---|
| ローカルTODOアプリ | 分類の編集/削除、削除確認UI、API失敗時の戻り方 | 普通に使う機能はある。さらに完成度を上げるなら、失敗した時や間違えた時の動きも必要 |
| UI/UX | mobile実機、キーボード操作、読み上げ、色コントラストの確認 | 例: iPhone幅で崩れない、Tabだけで操作できる、読み上げで意味が通る、文字が薄すぎないことを確認する |
| ハーネス実験素材 | 失敗ケース、再発検知、同じタスクの再試行データ | 例: 空タイトル、81文字タイトル、API失敗、title click誤動作、同じ依頼の再試行をチェックする |
| 再現性 | 総作業時間、各ステップの正確な時間、スクリーンショット、修正回数の記録 | 何をしたかは追える。どれだけ速く良くなったかを比べるには、時間と回数の記録がまだ足りない |
| 外部公開・本番運用 | 今回は対象外 | TODOアプリをSaaS扱いする話ではない。外部公開するなら別スコープで見る |

具体的にやること:

| 視点 | やること | OKの例 |
|---|---|---|
| UI/UX: mobile | `375px` 幅で作成、編集、詳細、分類バー、完了一覧を見る | 横にはみ出すのは分類バーだけ。modalは画面内に収まる |
| UI/UX: キーボード | マウスなしで `Tab`、`Enter`、`Space` だけで操作する | 新規作成を開ける。入力できる。保存できる。詳細を開閉できる。checkboxを切り替えられる |
| UI/UX: 読み上げ | checkbox、modal、エラーの名前を確認する | `買い物を完了にする`、`新しいTODO`、`タイトルは必須です` のように意味が分かる |
| UI/UX: 色 | 文字、badge、buttonの見やすさを見る | 薄い文字でも読める。色だけで状態を判断させない |
| ハーネス: 失敗ケース | わざと失敗する入力を入れる | 空タイトルは保存できない。81文字タイトルはエラー。API失敗時は理由か再試行が出る |
| ハーネス: 再発検知 | 過去に壊れた操作を固定テストにする | titleを押してもdoneにならない。編集を押しても詳細が同時に開かない |
| ハーネス: 再試行 | 同じ依頼をもう一度AIに出して比較する | 修正依頼回数、所要時間、Harness検出、人間検出が前回より良い |
