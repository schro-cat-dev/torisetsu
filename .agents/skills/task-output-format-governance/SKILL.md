...
調査ログ、講評ログ、検証ログ、設計メモ、判断ログなど、起点になる文書から派生したドキュメントを作る場合は、作成だけで完了にしない。

必ず同じタスク内で次を行う。

| 項目 | 書くこと | 例 |
|---|---|---|
| 起点文書 | どの文書から派生したか | `external_refs/ai_harness_skill_integration_references/README.md` |
| 派生文書 | 新しく作ったログや補足文書 | `2026-09-03-review-log.md` |
| 反映内容 | 起点文書へ戻した情報 | `除外候補、追加確認候補、講評ログへのリンク` |
| 辿り方 | 読者がどこから開けるか | READMEの `関連ログ` と `追加確認候補の扱い` |
| 完了確認 | 最後に何で確認したか | `sed -n`、`git diff` |

再現性の定義:

- 同じ種類の依頼では、毎回同じ詳細の粒度で出す。
- 同じ特徴を持つアウトプットを出す。
- 「リンクだけ」「感想だけ」「作成だけ」で終えない。
- 起点文書、派生文書、反映内容、確認方法が揃っている状態を完了とする。
...
| 反映内容 | 反映先 |
|---|---|
| `microsoft/SkillOpt` はrepoとして評価 1/10、除外扱い | `2026-09-03-review-log.md` |
| `addyosmani/agent-skills` は構造参照 | `追加確認候補の扱い` |
```
...
- [ ] 起点文書に派生文書へのリンクがある。
- [ ] 起点文書に、派生文書から戻した判断や要約がある。
- [ ] 派生文書に、事実、講評、扱いが分かれている。
- [ ] 会話ログに、作成・更新したファイルと判断内容がある。
- [ ] 最終回答で、更新ファイルと確認方法を伝える。
